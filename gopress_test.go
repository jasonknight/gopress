...
func TestCommentMetaCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.CommentId = 8699

		model.MetaKey = `lufidoyewoxetotodame`

		model.MetaValue = `lolukacupoporiharedo`
...
func TestCommentCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.CommentPostID = 43537

		model.CommentAuthor = `poyozonicatizoratosu`

		model.CommentAuthorEmail = `lilelewukuhocipesuyo`

		model.CommentAuthorUrl = `yasigulodazeliquvoso`

		model.CommentAuthorIP = `nisegicutakuqeluloli`

		d5 := NewDateTime()
		d5.FromString(`nisegicutakuqeluloli`)
		model.CommentDate = d5

		d6 := NewDateTime()
		d6.FromString(`nisegicutakuqeluloli`)
		model.CommentDateGmt = d6

		model.CommentContent = `buvizocuxiwudiwuqebo`

		model.CommentKarma = 2893

		model.CommentApproved = `nenapumihekugowawoyi`

		model.CommentAgent = `sakimayitoxatejegaza`

		model.CommentType = `kozaridizafagobayepe`

		model.CommentParent = 30347
...
func TestLinkCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.LinkUrl = `fiqotuvaqusozohoyepu`

		model.LinkName = `vafonibevupihinolohe`

		model.LinkImage = `taselobowimavewopapi`

		model.LinkTarget = `mepukeregaqopizegude`

		model.LinkDescription = `juneyocezikawopocuva`

		model.LinkVisible = `zekutolunuvesaboroci`

		model.LinkOwner = 39678

		model.LinkRating = 35820

		d8 := NewDateTime()
		d8.FromString(`35820`)
		model.LinkUpdated = d8

		model.LinkRel = `xatepuviwalacapazevi`

		model.LinkNotes = `jegasoxeverofotegeda`

		model.LinkRss = `neciwelapusiyogiroko`
...
func TestOptionCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.OptionName = `qedesaximahorovufiwu`

		model.OptionValue = `mimozesenogulojesite`

		model.Autoload = `puredetuzikonakifapu`
...
func TestPostMetaCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.MetaKey = `zocuwuwefusabigiteqo`

		model.MetaValue = `hokohiciqireyetitegu`
...
func TestPostCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.PostContent = `masovakobuyasoluroto`

		model.PostTitle = `lenuyoyanasawonabodi`

		model.PostExcerpt = `nuxixeyequdivecohudu`

		model.PostStatus = `tiremebasonujowumaxa`

		model.CommentStatus = `vuqiwalajozediqewuwo`

		model.PingStatus = `wihazuzefidiboredaru`

		model.PostPassword = `joyadajuqaqexeruvavo`

		model.PostName = `lanuqaqeyuvigikatugo`

		model.ToPing = `rawibahomovilojafofa`

		model.Pinged = `wopifiyocafuxebohulu`

		d13 := NewDateTime()
		d13.FromString(`wopifiyocafuxebohulu`)
		model.PostModified = d13

		d14 := NewDateTime()
		d14.FromString(`wopifiyocafuxebohulu`)
		model.PostModifiedGmt = d14

		model.PostContentFiltered = `gudeqihisiwomivifoka`

		model.PostParent = 26781

		model.Guid = `pofelodayulifukotuje`

		model.MenuOrder = 37486

		model.PostType = `bumiponixucumofinuho`

		model.PostMimeType = `vowabofewahebovubedo`

		model.CommentCount = 41321
...
func TestTermTaxonomyCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.Taxonomy = `nepajukemujarezozaje`

		model.Description = `xuregohapiyodejegefo`
...
func TestTermCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.Name = `vivahebulutecatuhomu`

		model.Slug = `wimesovobapohufijofe`

		model.TermGroup = 1796
...
func TestUserMetaCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.MetaKey = `burorelagatapovawaki`

		model.MetaValue = `qiqequfezizehotijeno`
...
func TestUserCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.UserLogin = `weducihorofotuzokuxi`

		model.UserPass = `yeqinijolizivekemoyi`

		model.UserNicename = `diracizuqeyewatihutu`

		model.UserEmail = `tudoxofofeqarayopuwo`

		model.UserUrl = `qivibiwinonevaduzise`

		d5 := NewDateTime()
		d5.FromString(`qivibiwinonevaduzise`)
		model.UserRegistered = d5

		model.UserActivationKey = `juyodoyimafidebisika`

		model.UserStatus = 31897

		model.DisplayName = `lanolewibafefuxehiyu`
...
func TestWooAttrTaxonomieCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.AttrName = `nemohapabadevebejici`

		model.AttrLabel = `duhifomedewexavayini`

		model.AttrType = `yorudavimofidimecaho`

		model.AttrOrderby = `yiperelamovomukemico`
...
func TestWooDownloadableProductPermCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.DownloadId = `hodazuxelopihumevoho`

		model.ProductId = 20910

		model.OrderId = 7742

		model.OrderKey = `yeduvufocijobaviwizi`

		model.UserEmail = `didipenitomuyusowidi`

		model.UserId = 22080

		model.DownloadsRemaining = `vujawoqebezesaporecu`

		d7 := NewDateTime()
		d7.FromString(`vujawoqebezesaporecu`)
		model.AccessGranted = d7

		d8 := NewDateTime()
		d8.FromString(`vujawoqebezesaporecu`)
		model.AccessExpires = d8

		model.DownloadCount = 3542
...
func TestWooOrderItemMetaCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.OrderItemId = -2542

		model.MetaKey = `nawefuminagumikedofi`

		model.MetaValue = `caquduriwupayikuwaxe`
...
func TestWooOrderItemCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.OrderItemName = `gerumadokisazasiqeja`

		model.OrderItemType = `pevuxizinubirixedula`
...
func TestWooTaxRateLocationCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.LocationCode = `pimuliyeyuqemipizoye`

		model.TaxRateId = 40366

		model.LocationType = `jotaseqagiragifacixe`
...
func TestWooTaxRateCreate(t *testing.T) {
	if fileExists(`../gopress.db.yml`) {
		...
		model.TaxRateCountry = `loridafofumiboyumadu`

		model.TaxRateState = `nenunawehihujuyuseyi`

		model.TaxRate = `nutiludopawesijiveja`

		model.TaxRateName = `tomedoxuxeheputevace`

		model.TaxRatePriority = 35874

		model.TaxRateCompound = 23688

		model.TaxRateShipping = -1593

		model.TaxRateOrder = 2279

		model.TaxRateClass = `tihibowoxasefosifaji`
